"""
Build r2: P1(DWM) + P2(freeze) + P3(taskbar collision with elastic rebound)
         + P4a(damping 0.997→0.9985) + P4b(rest delay 0.3→0.8s)
         + P4c(micro-perturbation, STACK-BALANCE FIXED)

Fixes from r1:
  - P4c: sin1 stored to ox_var before computing sin2 (was: sin1 left on stack,
    causing f*0.102 to multiply sin1 instead of f → stack underflow at stloc ox_var)
  - P4c: Block 2 reloads f from phase_field (was: relied on f passed through
    from Block 1, but Block 1 consumed f)
...
OUT  = os.path.join(BASE, "恋恋挂件_r2.exe")
...
os.remove(T)
print(f"\nDone: {OUT}")
print("Patches: P1(DWM) P2(freeze) P3(taskbar+screen+elastic) P4a(damping) P4b(rest) P4c(micro-perturb-FIXED)")
